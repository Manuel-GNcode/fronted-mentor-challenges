  ...
  return (
    <>
      <div className="flex flex-col gap-2 text-sm">
        {itemList.map(item => {
          return (
            <div className="flex justify-between items-center border-b border-b-cart-Rose-100 pb-2">
              <div>
                <h2 className="font-semibold text-cart-Rose-900">{item.name}</h2>
                <p key={item.name} className="space-x-2">
              ...
              <button className="border border-cart-Rose-500 p-1 rounded-full cursor-pointer h-fit"><img src="images/icon-remove-item.svg" alt="remove button" /></button>
            </div>
          )
        })}
      </div>
      <div className="flex justify-between my-4 text-sm font-semibold text-cart-Rose-500"><p>Order Total</p> <b className="text-cart-Rose-900">${itemList.reduce((acc, crr)=>acc+(crr.quantity*crr.price), 0).toFixed(2)}</b></div>
